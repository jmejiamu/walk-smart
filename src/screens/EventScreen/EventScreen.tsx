...
const EventScreen = () => {
  ...
  const {auth, eventInfo, joined, joinedEvent, getEventByID, joinEvent} =
    useContext(EventCtx);
  ...
  return (
    <View>
      <Btn
        ...
        children={<Icon name="arrow-left" color="#fff" size={30} />}
      />
      {joined.joined && (
        <Alert
          ContainerStyle={{
      ...
      <EventHeader eventName={eventInfo.event[0]?.event_title} />
      <ScrollView
        showsVerticalScrollIndicator={false}
        style={{
          ...styles.scrollStyle,
          height: Platform.OS === 'android' ? height * 0.26 : height * 0.33,
        }}>
        ...
        <ButtonComponent
          ...
          onPress={() =>
            joinEvent(
              auth.record.user_id,
              params.event_id,
              eventInfo.event[0].user_id ?? '',
              eventInfo.event[0],
            )
          }
        />
        <View style={styles.joinContainer}>
          <Icon name="users" size={30} color={colors.color_200} />
